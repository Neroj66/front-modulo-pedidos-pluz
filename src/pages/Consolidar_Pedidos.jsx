...
import { formatDateForFilename} from '../assets/js/functions';


const Consolidar_Pedidos = ({ username }) => {
  ...
  /*const getPedidos = () => {
    Axios.get("https://backend-modulo-pedidos.azurewebsites.net/consolidador/pedidos").then((response) => {
  ...
  const getPedidos = () => {
    ...
    Axios.get("https://backend-modulo-pedidos.azurewebsites.net/consolidador/pedidos", { params })
      .then((response) => {
        setPedidos(response.data);
...
const filteredSectores = showAll
  ...
  const csvReport = {
    data: mappedItems,
    headers: headers,
    filename: `pedidos-individuales_${dateFrom ? `desde_${formatDateForFilename(dateFrom)}` : 'sin_fecha_desde'}_${dateTo ? `hasta_${formatDateForFilename(dateTo)}` : 'sin_fecha_hasta'}.csv`
  };
  ...
  const exportToExcel = async () => {
    ...
    saveAs(blob, `pedidos individuales_${dateFrom ? `desde_${formatDateForFilename(dateFrom)}` : 'sin_fecha_desde'}_${dateTo ? `hasta_${formatDateForFilename(dateTo)}` : 'sin_fecha_hasta'}.xlsx`);
  };
    ...
    <div className="container">
      <div className="card text-center">
        <div className="card-header">
          Consolidar Pedidos Individuales
        </div>
        <div>
      ...
      <thead>
        <tr>
          ...
          <th  scope="col" 
                  ...
                  className={sortConfig.key === 'LCL_ING' ? (sortConfig.direction === 'asc' ? 'asc' : 'desc') : ''}>LCL</th>
          <th  scope="col" 
...
export default Consolidar_Pedidos;
